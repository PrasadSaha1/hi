'''
Notes -
There are references to an ability to export the user's grades as a PDF. That was never finished but left in the program
...
When exiting out of the program, close the window rather than stopping the program to make sure data is saved correctly.
'''


#importing tkinter and the messagebox module. Everything from tkinter is imported for convenience
from tkinter import *
...
#from reportlab.lib.pagesizes import letter #needed for exporting data, which was never finished
...
class GPACalculator:
    ...
    def add_new_class_quarterly_func(self):
        ...
        if len(self.class_data_quarterly) < 9: #A user can not have more than 9 classes to prevent widgets from overlapping, and this would not happen in a quarter
    ...
    def get_class_data(self):
        ...
        if screen in ["cumulative_grades", "more_8", "more_9", "more_10", "more_11", "more_12"]: #prevents data for quarterly grades entering when on cumulative screen
    ...
    def whats_my_gpa_func(self):
        ...
        if self.class_data_quarterly and screen == "quarterly_grades":
            can_calculate = True
        elif self.class_data_cumulative and screen in ["cumulative_grades", "class_term_grades_chooser_8", "class_term_grades_chooser_9", "class_term_grades_chooser_10", "class_term_grades_chooser_11", "class_term_grades_chooser_12"]:
            can_calculate = True
        if can_calculate:
...
class LoadingData:
    ...
    def __init__(self):
        ...
        self.save_data_button = Button(window, text="Save Data as PDF", command=self.save_data_func, font=(FONT, 30),
                                  bg=BACKGROUND_COLOR, fg=TEXTCOLOR, activebackground=BACKGROUND_COLOR, activeforeground=TEXTCOLOR)
        #note that the save_data_button is connected to exporting the data, which has not been finished yet
    ...
    def delete_data_func(self):
        ...
        if not answer:
            pass
        if answer:
            file_append.truncate(0) #delete file
...
def update_ui():
    ...
    if screen not in ["class_term_grades_chooser_8", "class_term_grades_chooser_9", "class_term_grades_chooser_10", "class_term_grades_chooser_11", "class_term_grades_chooser_12"]:
        if gpa_calculator.term_info_widgets:
            print(gpa_calculator.term_info_widgets)
    ...
    if screen == "data_menu":
        ...
        #data_menu.save_data_button.place(x=139 + x_location_change, y=250) - export data was never finished
    else:
        #deltes most of the widgets
