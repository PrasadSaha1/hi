...
import ast #for parsing the file
...
#to access docstrings, put help(FUNCTION/CLASS NAME) at the very end of the code, then exit out of the window and look at the output area
...
try:
    ...
    error_sign = Label(window, bg="red", width=2) #this will trigger another TclError

#if there is a problem with importing the image, even with the exception handling, comment out the error_sign lines (4 lines with the exception handling)
#and comment out the four lines in the four loop that create the error_label, with the exception handling
...
class HomeScreen:
    ...
    def calc_my_gpa_button_func(self) -> None:
    ...
    def data_information_func(self) -> None:
        '''When the button is pressed, the screen will change to the data menu'''
        global screen
        screen = "data_menu"

    def universal_back_button_func(self) -> None:
...
class GPACalculator:
    ...
    def __init__(self) -> None:
        ...
        self.class_ID = 1 #each of the user's classes will have this to help identify the class
    ...
    def quarterly_button_func(self) -> None:
    ...
    def cumulative_button_func(self) -> None:
    ...
    def more_8_func(self) -> None:
        '''Opens the 8th Grade Screen'''
        global screen
        screen = "more_8"

    def more_9_func(self) -> None:
        '''Opens the 9th Grade Screen'''
        global screen
        screen = "more_9"

    def more_10_func(self) -> None:
        '''Opens the 10th Grade Screen'''
        global screen
        screen = "more_10"

    def more_11_func(self) -> None:
        '''Opens the 11th Grade Screen'''
        global screen
        screen = "more_11"

    def more_12_func(self) -> None:
        '''Opens the 12th Grade Screen'''
        global screen
        screen = "more_12"

    def change_scale_func(self) -> None:
    ...
    def delete_class_quarterly_func(self) -> None:
    ...
    def add_class_cumulative_func(self, year: int) -> None: #takes a parameter, which is the year
        ...
        if len(year_classes) < 17: #confirms there are less than 17 classes to prevent widgets from overlapping
            ...
            self.class_data_cumulative.append([(class_entry, x_var_cumulative, y_var_cumulative),
                                   ...
                                   (more_info, x_var_cumulative + 336, y_var_cumulative),
                                    year, ["", str(self.default_Q1)], ["", str(self.default_Q2)], ["", str(self.default_E2)], ["", str(self.default_Q3)], ["", str(self.default_Q4)], ["", str(self.default_E4)], "", self.class_ID])
            ...
            self.for_export.append([["", x_var_cumulative, y_var_cumulative],
                                               ...
                                               ["", str(self.default_Q4)], ["", str(self.default_E4)], "", self.class_ID])

            self.class_ID += 1

    def delete_class_cumulative_func(self, year: int) -> None:
        ...
        if classes_of_year: #confirms that there is a class for this year
            ...
            for Class in self.for_export:
                if classes_of_year[-1][13] == Class[13]:
    ...
    def more_info_func(self, Class: int) -> None: #the parameter is the ID of the class
        ...
        self.terms_help_text = Label(window, text="This is an optional way to get more exact grades by using your term grades instead of your F4 grade. "
                                                  "\nWhen you hit save, your grade will now be determined with the term grades "
                                                  "\n and can’t be edited without editing the term grades. "
                                                  "\nIf you hit save but then decide to not calculate your grade using term grades, "
                                                  "\ndelete all the term grades and click save.",
                                                font=(FONT, 12), bg=BACKGROUND_COLOR, fg=TEXTCOLOR)
        self.term_grades_top = Label(window, text="Term Grade Calculator", font=(FONT, 20), bg=BACKGROUND_COLOR, fg=TEXTCOLOR)
        self.term_grade_class_name = Label(window, text=f"Class Name: {class_name}", font=(FONT, 20), bg=BACKGROUND_COLOR, fg=TEXTCOLOR)
    ...
    def place_widgets_in_more_info(self):
        ...
        self.terms_text.place(x=263 + x_location_change, y=300)
        self.term_grade_label.place(x=262 + x_location_change, y=343)
        self.term_weight_label.place(x=331 + x_location_change, y=343)

        #note that the label is at 224, grade entry at 256, and weight entry at 313
        self.Q1.place(x=224 + x_location_change, y=370)
        self.Q1_entry.place(x=256 + x_location_change, y=370)
        self.Q1_weight.place(x=313 + x_location_change, y=370)

        self.Q2.place(x=224 + x_location_change, y=400)
        self.Q2_entry.place(x=256 + x_location_change, y=400)
        self.Q2_weight.place(x=313 + x_location_change, y=400)

        self.E2.place(x=224 + x_location_change, y=430)
        self.E2_entry.place(x=256 + x_location_change, y=430)
        self.E2_weight.place(x=313 + x_location_change, y=430)

        self.Q3.place(x=224 + x_location_change, y=460)
        self.Q3_entry.place(x=256 + x_location_change, y=460)
        self.Q3_weight.place(x=313 + x_location_change, y=460)

        self.Q4.place(x=224 + x_location_change, y=490)
        self.Q4_entry.place(x=256 + x_location_change, y=490)
        self.Q4_weight.place(x=313 + x_location_change, y=490)

        self.E4.place(x=224 + x_location_change, y=520)
        self.E4_entry.place(x=256 + x_location_change, y=520)
        self.E4_weight.place(x=313 + x_location_change, y=520)
    ...
    def save_button_func(self, Class: int) -> None: #the parameter is the ID of the class
        ...
        for term in entry_weight: #each term
            ...
            try:
                if term[0] != "" and term[1] != "":
                    float(term[0]) #sees if it's a float or not
                    float(term[1])
                ...
                messagebox.showerror("Making Class Error", "Terms and percents must be positive numbers, with no % sign")
                break
        else:
            for users_class in self.class_data_cumulative:
                if users_class[13] == Class: #matches class ID
            ...
            try:
                ...
                for users_class in self.for_export:
                    if users_class[13] == Class: #appends the grade to for_export
                        users_class[12] = str(sum(grades) / sum(weights))
            except ZeroDivisionError: #there are no terms
                messagebox.showinfo("Making Class", "The grade for this class will no longer be calculated with the term grades")
                #the next few lines will clear users_class 12, which will remove the exact grade from the lists, so the grade will no longer be based on term grades
                for users_class in self.class_data_cumulative:
                    if users_class[13] == Class: #matches class ID
                        users_class[1][0].delete(0, END) #deletes the grade from the screen
                        users_class[12] = ""
                for users_class in self.for_export:
                    if users_class[13] == Class: #clears the grade from for_export
                        users_class[12] = ""

    def get_class_data(self) -> None:
    ...
    def whats_my_gpa_func(self): #likely to return none, but could return string if an error arises
        ...
        if can_calculate:
            if self.gpa_scale == 100:
                ...
                try:
                    ...
                    #the next line checks if a string is a float
                    for Class in self.data2: #raises an error if an input is negative
                        if Class[1] not in self.grades_4: #makes sure it isn't a letter grade
                            if float(Class[1]) < 0 or float(Class[3]) < 0:
                                raise ValueError
                    for Class in self.data2:
                        if Class[1] in self.grades_4: #if it is a letter grade, Class[1] is the grade
                            ...
                            #this is if a numerical grade was inputted
                    ...
                    messagebox.showerror("Calc GPA Error", "Please fill out all fields correctly. In addition, if you are loading your data in, please check each year before calculating your GPA.")
                    return "error" #returns error to prevent a PDF from being downloaded
                except ZeroDivisionError: #if weight or credit is zero
                    messagebox.showerror("Calc GPA Error", "Weight and Credit can not be zero")
                    return "error" #returns error to prevent a PDF from being downloaded
...
class HelpMenu:
    ...
    def __init__(self) -> None:
        ...
        self.Q5 = Label(window, text="Question: What 8th Grade classes count for High School GPA? \nAnswer: Only enter in Algebra I, Biology, and World Language for 8th Grade.",
                         font=(FONT, 12), bg=BACKGROUND_COLOR, fg=TEXTCOLOR, justify="left")
        self.Q6 = Label(window, text="Question: Where can I access the grades for classes I took in previous years? \nAnswer: Click Grade History in PowerSchool and click the respective year. Term grades are not saved.",
                         font=(FONT, 12), bg=BACKGROUND_COLOR, fg=TEXTCOLOR, justify="left")
        self.Q7 = Label(window, text="Question: How can I save my data?\nAnswer: Data is saved automatically with no input needed by the user. \nThe Data Information screen allows you to load in and delete previous data. \nIt also allows you to upload a PDF, but you need to calculate your cumulative GPA before doing so.",
                        font=(FONT, 12), bg=BACKGROUND_COLOR, fg=TEXTCOLOR, justify="left")
        self.Q8 = Label(window, text="Question: How can I report a bug for this program or ask additional questions? \nAnswer: Bugs and additional questions can be sent to prasadsaha11@gmail.com.",
                         font=(FONT, 12), bg=BACKGROUND_COLOR, fg=TEXTCOLOR, justify="left")
        ...
        self.settings_top = Label(window, text="Settings", font=(FONT, 30), bg=BACKGROUND_COLOR, fg=TEXTCOLOR)
        self.change_class_weight_top = Label(window, text="Change Class Weight", font=(FONT, 20), bg=BACKGROUND_COLOR, fg=TEXTCOLOR)
        self.change_term_default_weight_top = Label(window, text="Change Default Term Weight", font=(FONT, 20), bg=BACKGROUND_COLOR, fg=TEXTCOLOR)
    ...
    def instructions_func(self) -> None:
    ...
    def change_page_func(self) -> None:
    ...
    def confirm_settings_button_func(self) -> None:
        ...
        try:
            ...
            values = [float(help_menu.change_honors_weight_100_entry.get()), float(help_menu.change_AP_weight_100_entry.get()), float(help_menu.change_honors_weight_4_entry.get()),
                float(help_menu.change_AP_weight_4_entry.get()), float(help_menu.change_Q1_weight_entry.get()), float(help_menu.change_Q2_weight_entry.get()),
                float(help_menu.change_E2_weight_entry.get()), float(help_menu.change_Q3_weight_entry.get()), float(help_menu.change_Q4_weight_entry.get()), float(help_menu.change_E4_weight_entry.get())]

            for value in values:
                if value < 0: #confirms the values are positive
                    raise ValueError
...
class DataInformation:
    ...
    def __init__(self) -> None:
        ...
        self.data_info_top = Label(window, text="Data Information",
                              font=(FONT, 30), bg=BACKGROUND_COLOR, fg=TEXTCOLOR)
        self.save_data_button = Button(window, text="Save Data", command=self.save_data_func, font=(FONT, 30),
                                  bg=BACKGROUND_COLOR, fg=TEXTCOLOR, activebackground=BACKGROUND_COLOR, activeforeground=TEXTCOLOR)
        ...
        self.delete_data_button = Button(window, text="Delete Data", command=self.delete_data_func, font=(FONT, 30),
                                  bg=BACKGROUND_COLOR, fg=TEXTCOLOR, activebackground=BACKGROUND_COLOR, activeforeground=TEXTCOLOR)
        self.upload_PDF_button = Button(window, text="Create PDF", command=self.upload_PDF_func, font=(FONT, 30),
                                  bg=BACKGROUND_COLOR, fg=TEXTCOLOR, activebackground=BACKGROUND_COLOR, activeforeground=TEXTCOLOR)

    def save_data_func(self) -> None:
        messagebox.showinfo("Saving Data", "Data is saved automatically")

    def load_data_func(self) -> None:
        ...
        if os.path.getsize("data_for_calc_gpa.txt") and not data_loaded: #if the file isn't empty and hasn't been loaded in yet
            ...
            try: #this can be any type of wrong data, such as NameError. This would likely happen if the user directly edited the filed
                previous_data_all = ast.literal_eval(previous_data_all[0]) #if data is invalid, it won't be imported
                ...
                for index, Class in enumerate(previous_data_classes):
                    ...
                    temporary_ID = gpa_calculator.class_ID

                    #this lambda function has a number as a parameter rather than a dynamic variable that would yield inaccurate results.
                    more_info = Button(window, text="More", command=lambda temporary_ID=temporary_ID: gpa_calculator.more_info_func(Class=temporary_ID),
                    ...
                    gpa_calculator.class_data_cumulative.append([(small_class_entry, previous_data_classes[index][0][1], previous_data_classes[index][0][2]),
                                                     ...
                                                     (weight_menu, previous_data_classes[index][2][1], previous_data_classes[index][2][2]),
                                                      ...
                                                      [previous_data_classes[index][8][0], previous_data_classes[index][8][1]], [previous_data_classes[index][9][0], previous_data_classes[index][9][1]],
                                                       [previous_data_classes[index][10][0], previous_data_classes[index][10][1]], [previous_data_classes[index][11][0], previous_data_classes[index][11][1]], previous_data_classes[index][12], temporary_ID])
                    ...
                    gpa_calculator.for_export.append([[previous_data_classes[index][0][0], previous_data_classes[index][0][1], previous_data_classes[index][0][2]],
                                                     ...
                                                     [previous_data_classes[index][2][0], previous_data_classes[index][2][1], previous_data_classes[index][2][2]],
                                                       ...
                                                       previous_data_classes[index][5], [previous_data_classes[index][6][0], previous_data_classes[index][6][1]], [previous_data_classes[index][7][0], previous_data_classes[index][7][1]],
                                                       [previous_data_classes[index][8][0], previous_data_classes[index][8][1]], [previous_data_classes[index][9][0], previous_data_classes[index][9][1]],
                                                       [previous_data_classes[index][10][0], previous_data_classes[index][10][1]], [previous_data_classes[index][11][0], previous_data_classes[index][11][1]], "", temporary_ID])
                    gpa_calculator.class_ID += 1

                messagebox.showinfo("Loading in Data", "Data loaded in successfully")
                file_append.truncate(0)  # clears the file
            except Exception as e: #can be syntax error, type error, and more
                if not isinstance(e, KeyboardInterrupt): #all errors besides Keyboard Interrupt
    ...
    def delete_data_func(self) -> None:
        '''This function deletes the data from the file (but not from anywhere else)'''
        answer = messagebox.askyesno("Delete Saved Data", "To delete your saved data, you will have to exit the program and reopen it. Are you sure you want to exit the program to delete your data?")
        if not answer:
            pass
        if answer:
            file_append.truncate(0) #clear file
...
def display_classes(year, first_frame_more_year, first_frame_previous_more_year) -> None:
                ...
                if previous_data_classes and first_frame_previous_more_year[0]: #if there was previous data not placed yet
                    try: #if there is previous data for a class but none for the classes of the grade, there will be an index error
                        ...
                        if index == 1 and not Class[12]:
                            try: #grade of class
                                if previous_data_classes[i][1][0] in gpa_calculator.grades_4 and gpa_calculator.gpa_scale == 4:
                                    part[0].insert(0, previous_data_classes[i][1][0]) #letter grade
                                else:
                                    part[0].insert(0, str(round(float(previous_data_classes[i][1][0]))))
                            except Exception as e: #may cause error if there's an empty grade
                                if not isinstance(e, KeyboardInterrupt): #open for all errors besides keyboard interrupt
                                    pass
                    ...
                    except Exception as e: #IndexError, but just in case there is another error it is a bare except, excluding Keyboard Interrupt
...
def update_ui() -> None:
    ...
    if screen == "instructions_2":
        ...
        help_menu.Q4.place(x=0 + x_location_change, y=240)
        help_menu.Q5.place(x=0 + x_location_change, y=320)
        help_menu.Q6.place(x=0 + x_location_change, y=380)
        help_menu.Q7.place(x=0 + x_location_change, y=440)
        help_menu.Q8.place(x=0 + x_location_change, y=540)
        ...
        if screen != "instructions_1":
            help_menu.instructions_top.place_forget()
        for widget in [help_menu.Q3, help_menu.Q4, help_menu.Q5, help_menu.Q6,
                       help_menu.Q7, help_menu.Q8, help_menu.previous_page]:
            widget.place_forget()
    ...
    if screen == "settings":
        ...
        help_menu.change_class_weight_top.place(x=181 + x_location_change, y=85)
        help_menu.change_honors_weight_100_label.place(x=198 + x_location_change, y=125)
        help_menu.change_AP_weight_100_label.place(x=211 + x_location_change, y=155)
        help_menu.change_honors_weight_4_label.place(x=205 + x_location_change, y=185)
        help_menu.change_AP_weight_4_label.place(x=218 + x_location_change, y=215)

        # this places the entries for changing Honors and AP weight
        help_menu.change_honors_weight_100_entry.place(x=352 + x_location_change, y=125)
        help_menu.change_AP_weight_100_entry.place(x=337 + x_location_change, y=155)
        help_menu.change_honors_weight_4_entry.place(x=343 + x_location_change, y=185)
        help_menu.change_AP_weight_4_entry.place(x=331 + x_location_change, y=215)

        #this places the labels for changing default term weights
        help_menu.change_term_default_weight_top.place(x=138 + x_location_change, y=255)
        help_menu.change_Q1_weight_label.place(x=214 + x_location_change, y=295)
        help_menu.change_Q2_weight_label.place(x=214 + x_location_change, y=325)
        help_menu.change_E2_weight_label.place(x=216 + x_location_change, y=355)
        help_menu.change_Q3_weight_label.place(x=214 + x_location_change, y=385)
        help_menu.change_Q4_weight_label.place(x=214 + x_location_change, y=415)
        help_menu.change_E4_weight_label.place(x=208 + x_location_change, y=445)

        #this places the entries for changing default term weights
        help_menu.change_Q1_weight_entry.place(x=329 + x_location_change, y=295)
        help_menu.change_Q2_weight_entry.place(x=329 + x_location_change, y=325)
        help_menu.change_E2_weight_entry.place(x=327 + x_location_change, y=355)
        help_menu.change_Q3_weight_entry.place(x=329 + x_location_change, y=385)
        help_menu.change_Q4_weight_entry.place(x=329 + x_location_change, y=415)
        help_menu.change_E4_weight_entry.place(x=334 + x_location_change, y=445)
        ...
        for widget in [help_menu.settings_top, help_menu.change_class_weight_top, help_menu.change_honors_weight_100_label, help_menu.change_honors_weight_4_label, help_menu.change_AP_weight_100_label, help_menu.change_AP_weight_4_label,
                       help_menu.change_honors_weight_100_entry, help_menu.change_honors_weight_4_entry, help_menu.change_AP_weight_100_entry, help_menu.change_AP_weight_4_entry, help_menu.change_term_default_weight_top,
    ...
    if screen == "data_menu":
        ...
        data_menu.save_data_button.place(x=202 + x_location_change, y=70)
        data_menu.load_data_button.place(x=200 + x_location_change, y=160)
        data_menu.delete_data_button.place(x=190 + x_location_change, y=250)
        data_menu.upload_PDF_button.place(x=190 + x_location_change, y=340)
    else:
        #deletes most of the widgets
        for widget in [data_menu.data_info_top, data_menu.save_data_button, data_menu.load_data_button, data_menu.delete_data_button, data_menu.upload_PDF_button]:
            widget.place_forget()

def improve_entry_boxes(event) -> None:
...
def save_data_when_program_finished(sig, frame) -> None:
...
def update_frame() -> None:
    ...
    for entry in entry_boxes:
        if entry.winfo_ismapped(): #only entry boxes on the screen
            ...
            if entry.winfo_x() == 256 + x_location_change: #grade in more info:
            ...
            elif entry.winfo_x() == 254 + x_location_change: #if the widget is a grade entry
                if entry.get() == "": #if the widget is empty, place the error sign at the end of the widget
                    ...
                    error_labels[i].place_forget()
            elif entry.winfo_x() == 100 + x_location_change: #class name entry
                error_labels[i].place_forget() #class name entry does not need input
            #next line is all other widgets
            elif entry.get() == "" or not isfloat(entry.get()): #if the entry is blank or if the entry isn't float
                error_labels[i].place(x=entry.winfo_x() + entry.winfo_width() - error_labels[i].winfo_width() - 5, y=entry.winfo_y() + 5)
                error_labels[i].lift()
            else:
...
labels_for_help_text_when_hover = [gpa_calculator.class_name_label_quarterly, gpa_calculator.grade_label_quarterly, gpa_calculator.weight_label_quarterly, gpa_calculator.credit_label_quarterly, #labels for entry boxes, quarter GPA screen
                                   ...
                                   help_menu.change_class_weight_top, help_menu.change_term_default_weight_top]
for label in error_labels:
    labels_for_help_text_when_hover.append(label)

def start_hover(event) -> None:
    ...
    if event.widget.winfo_ismapped(): #if on screen
        ...
        try:
            if event.widget in error_labels: #text for error labels
                if event.widget.winfo_x() == 358: #weight menu
                    widget_help_label.config(text="Please enter this")
                    lines = 1 #the amount of lines the help text will take, based on the amount of \n
            ...
            if event.widget in [gpa_calculator.class_name_label_quarterly, gpa_calculator.class_name_label_8, gpa_calculator.class_name_label_9,
                ...
                lines = 1
            elif event.widget == help_menu.change_class_weight_top:
                #in settings, changing weight of advanced classes
                widget_help_label.config(text=f"Changes the Weight of honors and AP classes")
                lines = 1
            elif event.widget == help_menu.change_term_default_weight_top:
                #in settings, changing default term weights
                widget_help_label.config(text="Within the term grade chooser in cumulative GPA, this changes the default value of each term\n"
                                              "This will only impact future classes")
                lines = 2
...
previous_data = False
try:
    if len(ast.literal_eval(file_read.read())) >= 2: #if previous data, besides the constants
        previous_data = True
except SyntaxError: #if there is nothing in the file
    pass

if len(gpa_calculator.for_export) == 0 and previous_data: #there is previous data that hasn't been loaded or deleted
    file_append.close()  # close the files
    file_read.close()
    sys.exit() #exit the program before the files get changed
