...
from PySide6.QtCore import Qt, QEvent, QObject, QTimer, QThread, Signal, Slot
import sys
import signal
...
class HoverEventFilter(QObject):
    ...
    def __init__(self, help_label, text, widget, scroll_area=None):
        super().__init__()
    ...
    def eventFilter(self, obj, event):
        if event.type() == QEvent.Enter:  # if the user hovers over the widget
            ...
            self.help_label.setStyleSheet("color: black; background-color: white;")
...
class ChooseClassQMessageBox(QDialog):
    ...
    def __init__(self, title, message, combo_options, parent=None):
        super().__init__(parent)
        self.setWindowTitle(title)
        self.setLayout(QVBoxLayout())
...
class Worker(QThread):
    ...
    def run(self):
        ...
        while not self.window.logged_in:
            time.sleep(0.1)
            self.update_label.emit("Loading, please wait...")  # Update the label text via a signal
...
class Window(QWidget):
    ...
    def __init__(self):
        ...
        self.loading_screen = LoadingScreen()  # for the loading screen
        ...
        self.help_labels = []  # this is used for changing the color of help labels
        self.loaded = False  # this is used for loading screens
        ...
        self.instructions_screen_scroll_area_QWidget.setFixedSize(580, 1700)  # size for the scroll area QWidget
        ...
        self.stacked_widget.addWidget(self.create_PDF_screen)  # 17
        self.stacked_widget.addWidget(self.export_to_spreadsheet_screen)  # 18
        ...
        self.CREATE_PDF_SCREEN = 17
        self.EXPORT_TO_SPREADSHEET_SCREEN = 18
    ...
    def create_hover_help_text(self, widget, text, scroll_area=None):
        ...
        event_filter = HoverEventFilter(self.hover_label, text, widget, scroll_area)
        ...
        for Class in self.user_classes:
            if Class.MORE_INFO_ID == id:  # if this is the correct class
                ...
                self.create_hover_help_text(term_weight_label, "Enter the weight of the term. \n"
                                                               ...
                                                               "but they will only apply for future classes. \n"
                                                               "If a term grade is blank, the term weight will be zero")
                ...
                if Class.imported_data:
                    for name, text in [(Q1_grade_line_edit, Class.Q1_grade),
                                       (Q2_grade_line_edit, Class.Q2_grade),
                                       (E2_grade_line_edit, Class.E2_grade),
                                       (Q3_grade_line_edit, Class.Q3_grade),
                                       (Q4_grade_line_edit, Class.Q4_grade),
                                       (E4_grade_line_edit, Class.E4_grade)]:
                        name.setText(text)
                    Class.imported_data = False
                ...
                # this is the button to submit term grades, retrieving data from the user's inputs
    ...
    def create_instructions_screen(self):
        ...
        self.center_widget(self.importing_local_data_instructions_text, 1440)
        ...
        self.center_widget(self.export_data_instructions_top_text, 1565)
        ...
        self.center_widget(self.exporting_data_instructions_text, 1610)
    ...
    def change_colors_func(self, first=False):
        ...
        if not first:  # if it's being run from the button in the settings menu
            self.validation_timer.stop()  # prevent a RunTimeError
        ...
        for combo_box in self.findChildren(QComboBox):  # change the color of the buttons, including when they are pressed
        ...
        if self.login_username_line_edit.text() in formatted_usernames:  # if username is valid
            ...
            if self.login_password_line_edit.text() == password:  # if password is valid
                ...
                if answer == QMessageBox.Yes:  # if no, nothing happens
                    ...
                    self.worker.update_label.connect(self.loading_screen.update_text)
                    self.worker.start()
    ...
    def create_create_PDF_screen(self):
        ...
        self.create_PDF_for_one_class_button = QPushButton("Create PDF of one class", self.create_PDF_screen)
        self.create_PDF_for_one_class_button.clicked.connect(self.create_PDF_for_one_class)
        self.font_size(self.create_PDF_for_one_class_button, 15)
        self.center_widget(self.create_PDF_for_one_class_button, 260)

    def draw_centered_text(self, text, y, c):  # used to draw text
        '''This function centralizes elements of the PDF'''
        width = c._pagesize[0]  # gets the width from [width, height]
        text_width = c.stringWidth(text)  # gets the width of the text to be centralized
        x = (width - text_width) / 2  # finds the x value
        c.drawString(x, y, text)  # draws the text
    ...
    def create_PDF_for_all_classes(self, scope):
        ...
        if runnable == "error":  # if GPA can't be calculated
            ...
            if confirm.exec():  # if yes
                ...
                self.draw_centered_text("Monroe-Woodbury High School Grade Report", y_coord, Canvas)
                y_coord -= 20  # the y coordinate decreases to make sure information doesn't overlap
                self.draw_centered_text(name, y_coord, Canvas)
                y_coord -= 20
                self.draw_centered_text(self.formatted_date, y_coord, Canvas)
                y_coord -= 10

                if scope == "Quarterly":
                    y_coord -= 20
                    self.draw_centered_text(f"{year_}th Grade", y_coord, Canvas)
                    ...
                    for year in range(8, 13):  # range() is exclusive, so it goes up to 13 instead of 12
                        ...
                        if classes_in_this_year:  # if there is at least one class for that grade
                            ...
                            self.draw_centered_text(f"{year}th Grade", y_coord, Canvas)
                            y_coord -= 20
                ...
                self.draw_centered_text(f"Unweighted GPA: {self.unweighted_gpa}", y_coord, Canvas)
                y_coord -= 20
                self.draw_centered_text(f"Weighted GPA: {self.weighted_gpa}", y_coord, Canvas)

                # information about weights
                y_coord -= 100
                self.draw_centered_text(f"Honors Scale: {self.honors_weight}", y_coord, Canvas)
                y_coord -= 20
                self.draw_centered_text(f"AP Scale: {self.AP_weight}", y_coord, Canvas)

                Canvas.save()
                webbrowser.open(file)  # opens the file

    def create_PDF_for_one_class(self):
        """This function creates a PDF for one class"""
        non_empty_classes = []
        for user_class in self.user_classes:
            if not (user_class.grade == user_class.weight == user_class.credit == ""):
                non_empty_classes.append(user_class)
        if len(non_empty_classes) == 0:  # if there are no classes
            ...
            if runnable == "error":
                for user_class in self.user_classes:
                    ...
                    if quarterly_classes == 0:  # only for cumulative
                        QMessageBox.critical(self, "Exporting Data error",
                                             "Ensure that you can calculate your Cumulative GPA before exporting data")
                    elif cumulative_classes == 0:  # only for quarterly
                        QMessageBox.critical(self, "Exporting Data error",
                                             "Ensure that you can calculate your Quarterly GPA before exporting data")
                    else:  # for both
                        QMessageBox.critical(self, "Exporting Data error",
                                             "Ensure that you can calculate your GPA on both the Quarterly and Cumulative scope before exporting data")
            else:
                class_names = []
                hidden_class_names = []  # includes the class ID
                for user_class in non_empty_classes:
                ...
                self.loaded = True
                self.validation_timer.start(250)  # prevent a RunTimeError
                if message_box.exec():
                    ...
                    for user_class in non_empty_classes:
                        if int(user_class.class_id) == int(hidden_class_names[selected_indices[0]]):  # if this is the correct class
                            ...
                            # information for the top
                            self.draw_centered_text("Monroe-Woodbury High School Class Report", y_coord, Canvas)
                            y_coord -= 20  # the y coordinate decreases to make sure information doesn't overlap
                            self.draw_centered_text(entered_text, y_coord, Canvas)
                            y_coord -= 20
                            self.draw_centered_text(self.formatted_date, y_coord, Canvas)
                            y_coord -= 50

                            # information about the class
                            self.draw_centered_text(f"Class Name: {user_class.class_name}", y_coord, Canvas)
                            y_coord -= 20
                            self.draw_centered_text(f"Grade: {user_class.grade}", y_coord, Canvas)
                            y_coord -= 20

                            try:
                                if float(user_class.weight) == float(self.honors_weight):
                                    weight = f"H ({self.honors_weight})"
                                elif float(user_class.weight) == float(self.AP_weight):
                                    weight = f"AP ({self.AP_weight})"
                                else:
                                    weight = "R"
                            except ValueError:  # sometimes the weight isn't converted
                                weight = str(user_class.weight)

                            self.draw_centered_text(f"Weight: {weight}", y_coord, Canvas)
                            y_coord -= 20
                            self.draw_centered_text(f"Credit: {user_class.credit}", y_coord, Canvas)
                            y_coord -= 50

                            # information about term grades. Each grade only shows if the grade was entered
                            if selected_indices[1] == 0:  # only for F4
                                for term_grade in [user_class.Q1_grade, user_class.Q2_grade, user_class.E2_grade,
                                                   user_class.Q3_grade, user_class.Q4_grade, user_class.E4_grade]:
                                    if term_grade:
                                        Canvas.drawString(200, y_coord, "Term Name")
                                        Canvas.drawString(275, y_coord, "Term Grade")
                                        Canvas.drawString(350, y_coord, "Term Weight")
                                        y_coord -= 20
                                        break  # ensure that this only occurs once
                                ...
                                for term, term_grade, term_weight in [["Q1", user_class.Q1_grade, user_class.Q1_weight], ["Q2", user_class.Q2_grade, user_class.Q2_weight],
                                    ...
                                    if term_grade:
                                        Canvas.drawString(200, y_coord, term)
                                        Canvas.drawString(275, y_coord, str(round(float(term_grade))))
                                        Canvas.drawString(350, y_coord, str(term_weight))
                                        y_coord -= 20
                            else:  # for a specific term
                                Canvas.drawString(200, y_coord, f"{selected_options[1]} Grade: ")
                                for term, term_grade in [("Q1", user_class.Q1_grade), ("Q2", user_class.Q2_grade),
                                    ...
                                    if selected_options[1] == term and term_grade:
                                        Canvas.drawString(300, y_coord, str(round(float(term_grade))))
                                        y_coord -= 20
                            if user_class.categories:
                                y_coord -= 50
                                self.draw_centered_text("Grading Categories", y_coord, Canvas)
                                y_coord -= 20
                                Canvas.drawString(200, y_coord, "Category Name")
                                Canvas.drawString(350, y_coord, "Category Weight")
                                y_coord -= 20

                                for category in user_class.categories:  # add each category
                                    Canvas.drawString(200, y_coord, category[0])
                                    Canvas.drawString(350, y_coord, str(category[1]))
                                    y_coord -= 20
                            ...
                            for term in ["Q1", "Q2", "E2", "Q3", "Q4", "E4"]:  # create assignments for each term
                                if selected_options[1] in ["All (F4/F2)", term]:  # if the term is all or Q1
                                    ...
                                    if user_class.current_term.assignments:
                                        Canvas.showPage()
                                        new_y_coord = 750

                                        new_y_coord -= 50
                                        self.draw_centered_text(f"{term} Assignments", new_y_coord, Canvas)
                                        new_y_coord -= 40

                                        Canvas.drawString(100, new_y_coord, "Category")
                                        Canvas.drawString(250, new_y_coord, "Assignment Name")
                                        Canvas.drawString(400, new_y_coord + 15, "Points")
                                        Canvas.drawString(400, new_y_coord, "Received")
                                        Canvas.drawString(470, new_y_coord + 15, "Points")
                                        Canvas.drawString(470, new_y_coord, "Available")
                                        Canvas.drawString(540, new_y_coord, "Grade")
                                        new_y_coord -= 20

                                        # entering in information about each assignment
                                        for assignment in user_class.current_term.assignments:
                                            Canvas.drawString(100, new_y_coord, assignment[0])
                                            Canvas.drawString(250, new_y_coord, assignment[1])
                                            Canvas.drawString(400, new_y_coord, str(assignment[2]))
                                            Canvas.drawString(470, new_y_coord, str(assignment[3]))
                                            ...
                                            Canvas.drawString(540, new_y_coord, str(assignment_grade))
                                            new_y_coord -= 20
    ...
    def SQL_dynamic_backup(self, final_run=False):
        ...
        while True:  # this runs in the background with a thread
            ...
            for user_class in self.user_classes:
                ...
                first_time_export = 1
                Q1_term_first_time_export = 1
                Q2_term_first_time_export = 1
                E2_term_first_time_export = 1
                Q3_term_first_time_export = 1
                Q4_term_first_time_export = 1
                E4_term_first_time_export = 1

                mappings = [
                    (user_class.first_time, 'first_time_export'),
                    (user_class.Q1.term_first_time, 'Q1_term_first_time_export'),
                    (user_class.Q2.term_first_time, 'Q2_term_first_time_export'),
                    (user_class.E2.term_first_time, 'E2_term_first_time_export'),
                    (user_class.Q3.term_first_time, 'Q3_term_first_time_export'),
                    (user_class.Q4.term_first_time, 'Q4_term_first_time_export'),
                    (user_class.E4.term_first_time, 'E4_term_first_time_export')
                ]

                for old, new_name in mappings:
                    if old is True:
                        globals()[new_name] = 1
                    else:
                        globals()[new_name] = 0
    ...
    def input_validation(self):
        ...
        try:
            ...
            # the validation timer is stop to prevent this error, but this is a backup
            pass
